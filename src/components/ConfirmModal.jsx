    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" onClick={onCancel}>
      <div className="w-full max-w-sm rounded-2xl bg-panel p-6 shadow-xl ring-1 ring-white/10" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={onCancel} className="flex-1 rounded-full border border-white/15 py-2.5 text-sm font-medium text-cream/70 hover:bg-white/5">
            Cancel
          </button>
          <button onClick={onConfirm} className="flex-1 rounded-full bg-cream py-2.5 text-sm font-semibold text-ink hover:bg-white">
            {confirmLabel}
          </button>
        </div>
      </div>
